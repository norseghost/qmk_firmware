...
bool process_achordion(uint16_t keycode, keyrecord_t* record) {
    ...
    const bool is_physical_pos = (record->event.key.row < KEYLOC_COMBO && record->event.key.col < KEYLOC_COMBO);

    if (achordion_state == STATE_RELEASED) {
        if (is_tap_hold && record->tap.count == 0 && record->event.pressed && is_physical_pos) {
    ...
    if (achordion_state == STATE_UNSETTLED && record->event.pressed) {
        ...
        if (!is_physical_pos || (is_tap_hold && record->tap.count == 0) || achordion_chord(tap_hold_keycode, &tap_hold_record, keycode, record)) {
